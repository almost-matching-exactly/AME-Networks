bin_me <- function(x, adjacent = TRUE) {
  # Takes in a factor x and returns a list, each entry of which is
  # that factor with two levels binned together
  # If adjacent = TRUE, will only consider binning adjacent levels; else, all
  
  bin_combs <- get_bin_combs(x, adjacent)
  all_binnings <- 
    lapply(bin_combs, function(y) {
      l1 <- y[1]
      l2 <- y[2]
      binned_name <- paste(l1, l2, sep = '')
      fct_collapse(x, !!quo_name(binned_name) := c(l1, l2))
    })
  return(all_binnings)
}

get_bin_combs <- function(x, adjacent = TRUE) {
  # Return possible pairs of levels to bin together
  # If adjacent == TRUE, only bins adjacent levels togethers; else, all levels
  ## Might be slightly slower to do it this way because if adjacent, looping twice
  levs <- levels(x)
  if (adjacent) {
    bin_combs <- 
      lapply(2:nlevels(x), function(y) c(levs[y - 1], levs[y]))
    return(bin_combs)
  }
  bin_combs <- combn(levs, 2, simplify = FALSE)
  return(bin_combs)
}

drop_unmatchable <- function(data) {
  # Called before every covariate is temporarily dropped and the result analyzed
  # Drops any covariates where there are only two levels of a factor, one of which is only
  ...
  dims <- dim(data)
  if (dims[1] == 2) { # If only two rows left 
    return(data)
  }
...
bin_update_matched_bit <- function(data, cur_covs, covs_max_list, compute_var) {
  data_wo_t <- as.bigz(as.matrix(data[, cur_covs+1])) # the covariates values as a matrix
...
bin_match_quality_bit <- function(c, data, holdout, num_covs, cur_covs, covs_max_list, tradeoff,
                              PE_function, model, ridge_reg, lasso_reg, compute_var) {
  
  cov_binnings <- bin_me(data[, c + 1], adjacent = TRUE)
  n_binnings <- length(cov_binnings)
  MQs <- vector(mode = 'numeric', length = n_binnings)
  tmp <- data
  for (i in 1:n_binnings) {
    tmp[, c + 1] <- cov_binnings[[i]]
    # Calculate number of units unmatched (available)
    ...
    BF <- if_else(num_control == 0 | num_treated == 0,
                  0,
                  num_control_matched / num_control + num_treated_matched / num_treated)
    MQs[i] <- tradeoff * BF - PE
  }
  best_binning <- which.max(MQs)
}
...
FLAME_bit <- function(data, holdout, tradeoff = 0.1, compute_var = FALSE, PE_function = NULL,
  ...
  if (sum(match_index) > 0) {
    matched_to <- lapply(1:length(uniq_index), 
                         function(x) which(match_index)[which(index == uniq_index[x])]) 
    # MGs[[length(MGs) + 1]] <- data[which(match_index), cur_covs + 1, drop = FALSE] %>%
  ...
  while (length(cur_covs) > n_cov_min && 
    ...
    ## Have to deal with sometimes dropping, sometimes binning 
  ...
  return_list = list(covs_list, CATE, unlist(SCORE), return_df, matched_to)
  names(return_list) = c("covariate_list", "matched_group", "match_quality", "matched_data", "MGs")
  return(return_list)
}